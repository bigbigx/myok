    ...
    def run(self):
        ...
        while True:
            if self.handshaken == False:  # 如果没有进行握手
                if PRINT_FLAG:
                    print('INFO: Socket %s Start Handshaken with %s! \n' % (self.index, self.remote))
                ...
                if self.length_buffer - g_header_length < g_code_length:
                    ...
                    if recv_message == "quit":
                        ...
                        nowTime = time.strftime('%H:%M:%S', time.localtime(time.time()))
                        chan = self.tailf_file()
                        cmd = "tailf /var/log/nginx/access.log"
                        chan.exec_command(cmd)
                        while True:
                            # clientdata = parse_data(self.conn.recv(1024).decode('utf-8'))
                            # print(clientdata)
                            # if clientdata == 'quit':  # 但浏览器点击stop按钮或close按钮时，断开连接
                            #     self.conn.close()
                            #     break
                            # print('test')
                            while True:
                                while chan.recv_ready():
                                    ...
                                    sendMessage("%s %s say: %s" % (nowTime, self.remote, log_msg))

                                    clientdata1 = self.conn.recv(1024).decode('utf-8')
                                    print('hello world')
                                    print(clientdata1)
                            ...
                            #     log_msg = chan.recv(10000).strip().decode() + '\n'  # 接收日志信息
                            #     sendMessage("%s %s say: %s" % (nowTime, self.remote, log_msg))
                            # tmp = parse_data(self.conn.recv(100))
                            # print(tmp)
                            # # tmp1 = bytes.decode(tmp)
                            # print(tmp)
