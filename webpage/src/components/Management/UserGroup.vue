...
<div>
  ...
  <Card>
    ...
    <div class="edittable-testauto-con">
      <Form :model="groupinfo" :label-width="80" ref="groupinfova" :rules="userinfoValidate">
        <FormItem label="群组名" prop="groupname">
          <Input v-model="groupinfo.groupname" placeholder="请输入"></Input>
        </FormItem>
        <FormItem label="群组说明" prop="remark">
          <Input v-model="groupinfo.remark" placeholder="请输入" ></Input>
        </FormItem>
        <FormItem label="选择成员" prop="members">
              <Tooltip placement="top">
              <Checkbox v-model="formGroupItem.user"  >选择(可多选)</Checkbox><p></p>
                <div slot="content">
                  <p>点击选择文件所属群组，可以多选</p>
              </div>
              </Tooltip>
              ...
              <CheckboxGroup  v-model="social">
                      <Checkbox v-for="k in this.item_user" :label="k.username">
                          <Icon type="social-twitter"></Icon>
                          <span >{{k.username}}</span>
                      </Checkbox>
                  </CheckboxGroup>
        </FormItem>
        <Button type="primary" @click.native="AddGroup" style="margin-left: 35%">添加</Button>
      </Form>
    </div>
  </Card>
  </Col>
  ...
  <Card>
    ...
    <div class="edittable-con-1">
      <Table border :columns="columns6" :data="groupdata" stripe height="550"></Table>
    </div>
    <br>
  ...
  <Modal v-model="editPasswordModal" :closable='false' :mask-closable=false :width="500">
    ...
    <Form ref="editPasswordForm" :model="editPasswordForm" :label-width="100" label-position="right" :rules="passwordValidate">
      <FormItem label="用户名">
        <Input v-model="groupname" readonly="readonly"></Input>
      </FormItem>
...
export default {
  ...
  methods: {
    ...
    AddGroup () {
      this.$refs['groupinfova'].validate((valid) => {
        if (valid) {
          axios.post(util.url + '/group/', {
    ...
    refreshuser (vl = 1) {
      axios.get(`${util.url}/group/all?page=${vl}`)
        .then(res => {
    ...
    saveEditInfo () {
      axios.put(util.url + '/group/changemembers', {
          'username': this.username,
    ...
    delUser () {
      if (this.groupname === this.confirmgroup) {
        axios.delete(util.url + '/group/' + this.username)
          .then(res => {
            this.$Notice.success({
              title: '通知',
              desc: res.data
